<template>
  <el-card class="box-card">
    ...
    <div class="table-box">
      <el-table element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="创建时间">
          <template #default="scope">
            <el-tooltip :content="formatTime(scope.row.create_time, 'yyyy-MM-dd')" placement="top" effect="light">
              <span class="highlight">{{ formatTime('{0}-{1}-{2}', scope.row.create_time) }}</span>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button size="small" @click="editGradeClass(scope.row.id)" style="margin: 0 0 10px 10px;">编辑</el-button>
            <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" icon-color="#626AEF" :icon="Delete"
              :title="'确定删除班级名为“' + scope.row.name + '”的班级吗？'" @confirm="delGradeClass(scope.row.id)">
              <template #reference>
    ...
    <el-pagination background layout="total, sizes, prev, pager, next, jumper" :total="total" v-model:page-size="pageSize"
      @size-change="sizeChange" @current-change="changePage" :page-sizes="[10, 20, 30, 40]" />
    <!--分页 end-->
  </el-card>
  <!--新增班级弹出框 start-->
  <el-dialog align-center v-model="addGradeClassDialogFormVisible" style="max-width: 650px;" width="90%" destroy-on-close>
  ...
  <!--编辑班级弹出框 start-->
  <el-dialog align-center v-model="editGradeClassDialogFormVisible" style="max-width: 650px;" width="90%"
    destroy-on-close>
    <template #header="{ close, titleId, titleClass }">
      <div class="my-header">
        <el-icon size="26px">
...
import { deleteGradeClassApi, getGradeClassApi, getGradeClassListApi } from "../../api/gradeclass"
...
const loadData = async (state) => {
  ...
  state.total = data.result.totalNum
  state.loading = false
}
const Nindex = (index) => {
  // 当前页数 - 1 * 每页数据条数 + 1
