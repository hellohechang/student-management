  ...
  <el-card class="box-card">
    ...
    <template #header>
      <div class="card-header">
        ...
        <!--搜索区域 start-->
        <div class="card-search" style="flex: auto;">
          <el-row style="justify-content: end;" :gutter="10">
            <el-col :span="4">
            ...
            <el-col :span="1" style="display: inline-flex;justify-content: center;align-items: center; cursor: pointer;">
    ...
    <div class="table-box">
      <el-table row-key="id"  element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="序号" width="60" type="index" :index="Nindex"/>
        ...
        <el-table-column prop="course_name" label="学科">
          <template #default="scope">
        ...
        <el-table-column label="创建时间">
          <template #default="scope">
            <el-tooltip :content="scope.row.create_time" placement="top" effect="light">
...
import {deleteScoresApi, editScoresApi, getScoresListApi, registerScoresApi} from "../../api/scores";
...
const registerScores = async () => {
  ...
  const { data } =  await registerScoresApi({gradeClassId:gradeClassId.value,courseId:courseId.value})
  if(data.status===200){
...
const celldblclick = (row, column, cell, event)=> {
  const scoreTarget = state.tableData.find(item => {
...
const exportExcelAction = () => {
  // 处理表格数据
  const newTableData = state.tableData.flatMap((item)=> {
