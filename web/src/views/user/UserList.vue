<template>
    ...
    <div class="table-box">
      <el-table element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button size="small" style="margin: 0 0 10px 10px;" @click="editUser(scope.row.id)">编辑</el-button>
            <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" :icon="Delete" icon-color="#626AEF"
              :title="'确定删除用户名为“' + scope.row.username + '”的用户吗？'" @confirm="delUser(scope.row.id)">
              <template #reference>
      ...
      <el-pagination background layout="total, sizes, prev, pager, next, jumper" :total="total"
        v-model:page-size="pageSize" @size-change="sizeChange" @current-change="changePage" :page-sizes="[10, 20, 30, 40]" />
      <!--分页 end-->
    </div>
    <!--表格区域 end-->
  </el-card>
  <!--新增用户弹出框 start-->
  <el-dialog style="max-width: 650px;"  align-center  v-model="userDialogFormVisible"  width="90%" destroy-on-close>
  ...
  <!--编辑用户弹出框 start-->
  <el-dialog style="max-width: 650px;" align-center v-model="editUserDialogFormVisible"  width="90%" destroy-on-close>
    <template #header="{ close, titleId, titleClass }">
      <div class="my-header">
        <el-icon size="26px"><EditPen /></el-icon>
...
import { deleteUserApi, getUserApi, getUserListApi } from "../../api/user"
