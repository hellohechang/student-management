    ...
    <el-row>
      <el-col>
        <el-form-item prop="roleId" label="所属角色">
          <el-select v-model="formUser.roleId" placeholder="请选择角色" style="width: 100%;">
...
import { editUserApi, getAllRoleListApi } from "../../../api/user"
...
let { formUser } = toRefs(data)
// 给表单填充数据
formUser.value = deepClone(userInfo.value)
// 定义表单约束规则对象
...
const editUser = async (formEl) => {
  ...
  await formEl.validate(async (valid, fields) => {
    ...
    if (valid) {
      const { data } = await editUserApi(formUser.value)
      if (data.status === 200) {
