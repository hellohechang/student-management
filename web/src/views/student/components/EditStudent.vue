<template>
  <el-form ref="ruleFormRef" :rules="rules" :model="formStudent" label-width="80px">
    <el-row>
      <el-col :xs="24" :sm="12">
        <el-form-item label="所属班级" prop="gradeClassId">
...
import { editStudentApi, gradeClassListApi } from "../../../api/student";
...
// 给表单填充数据
formStudent.value = deepClone(studentInfo.value)
// 定义表单约束规则对象
const rules = reactive({
  ...
  gradeClassId: [{ required: true, message: '班级不能为空', trigger: 'change' }],
  stuno: [{ required: true, message: '班级不能为空', trigger: 'blur' }],
})
// 编辑学生信息
const editStudent = async (formEl) => {
  if (!formEl) return
  await formEl.validate(async (valid, fields) => {
    subLoading.value = true
    if (valid) {
      const { data } = await editStudentApi(formStudent.value)
      if (data.status === 200) {
