...
route.get('/scoreslist', async (req, res) => {
  try {
    let { pageIndex = 1,
      ...
      searchValue = '',
      courseId,
      gradeClassId } = req.query;
    let scoresArr = []
    if (courseId && gradeClassId) {
      scoresArr = await queryData('s_student_score', '*', `WHERE course_id=? AND gradeclass_id=?`, [courseId, gradeClassId])
    } else {
...
route.post('/registerscores', async (req, res) => {
  try {
    let { gradeClassId,
      courseId } = req.body;
    let time = Date.now()
    let studentArr = await queryData('s_student', '*', `WHERE grade_class_id=?`, [gradeClassId])
...
route.put('/updatescores', async (req, res) => {
  try {
    let {
      ...
      score } = req.body;
    let time = Date.now()
    await updateData('s_student_score', {
      update_by: req._userInfo.id,
      update_time: time,
      score,
      type: '已批改'
